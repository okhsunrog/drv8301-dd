...
The driver provides direct access to all DRV8301 registers through the low-level API via `drv.ll`. This API is automatically generated from [`device.ddsl`](device.ddsl) and provides type-safe access to all register fields.
...
drv.ll.control_register_1().write(|w| {
    w.set_gate_current(GateCurrent::High);
    w.set_gate_reset(false);
    w.set_pwm_mode(false); // 6-PWM mode
...
drv.ll.control_register_2().write(|w| {
    w.set_octw_mode(OctwMode::OtAndOc);
    w.set_gain(ShuntAmplifierGain::Gain20);
    w.set_dc_cal_ch_1(false);
    w.set_dc_cal_ch_2(false);
    w.set_oc_toff(false);
})?;
```
...
drv.ll.control_register_2().modify(|w| {
    w.set_dc_cal_ch_1(true);  // Only modify this field, others preserved
})?;
```
...
- Field: `dc_cal_ch1` → `set_dc_cal_ch_1()` / `dc_cal_ch_1()` (digits are split into their own word)
...
The DRV8301 register map is defined in [`device.ddsl`](device.ddsl), which `device-driver` uses to generate Rust code. This file specifies:
